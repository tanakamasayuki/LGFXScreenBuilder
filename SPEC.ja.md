...
- Export Arduino: `ui_generated.h` または `ui_export.zip` をダウンロードする。
...
MVP では各機種は共通ベースレイアウトで描画する（サイズ差はクリップ／左上原点）。「単一機種できれいに動き、複数機種でも後付けで破綻しない」方針に沿い、プロファイル別の作り込みは MVP の範囲外とする。

MVP では以下を後回しにする。

- 複数プロファイルのレイアウト差分（オーバーライド）編集
- シーン別の回転上書き編集（MVP はプロファイルのデフォルト回転のみ）
...
- ZIP パッケージ出力（`.lgfxsb` / `ui_export.zip`）
