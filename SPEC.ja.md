...
- Part: Scene 上に配置され、描画される最小単位。Text、Image、Rect など。
- Group: Part または Group をまとめる階層ノード。
- Component: 複数 Part/Group を再利用するテンプレート。MVP では扱わず、将来拡張とする。

Design モードでは `Parts` セクションに追加可能な Part/Group を常時表示する。画面へ要素を追加する導線は `Parts` に統一し、Asset だけを特別扱いしない。
...
MVP の Group は、階層、名前空間、ローカル座標の基準を提供する論理ノードとする。
