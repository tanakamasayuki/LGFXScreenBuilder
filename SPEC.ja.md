...
- **集合はライブラリごとに異なる**。LovyanGFX と M5GFX はそれぞれ独自の `lgfx_fonts.hpp` を持ち（1.2.21 で約 186、M5GFX 0.2.22 で約 187。efont 系の日本語/中国語/韓国語などを含む）、共有フォントは同名・`namespace fonts::` で宣言される。対象ライブラリ（§9）に応じて候補を出し分ける（ボード割り当てと同じ流儀）。
...
- カタログ生成（パース＋host 内省＋サンプル PNG）は CI/手動で行い、結果の JSON（＋プレビュー画像）を成果物として持つ。ライブラリ更新時に再生成する。
...
- 対象ライブラリの `fonts::` に存在しないフォント（例：LovyanGFX に無い M5GFX 専用フォント）は**出力しない＋警告**する（ボード割り当てと同じ扱い、§8.9.5）。
