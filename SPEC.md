...
- Render-mode configuration (direct drawing or tiled double-buffered drawing through LGFXVirtualCanvas)
...
Export can select the actual-device render mode. The default is **tiled double-buffered rendering through LGFXVirtualCanvas**, reducing flicker between the background clear and subsequent part drawing. Direct drawing can be selected when compatibility or minimal dependencies matter more. Generated output that selects tiled double buffering requires `LGFXVirtualCanvas` as a dependent library, and the sample `.ino` includes `#include <LGFXVirtualCanvas.h>`. Generated output that selects direct drawing does not depend on `LGFXVirtualCanvas`.
...
namespace lgfxsb {
  ...
  protected:
    ...
    bool _buffered = true;         // true = LGFXVirtualCanvas tiled double buffer (depends on generated setting)
  ...
  public:
    ...
    void setBuffered(bool enable);  // can switch back to direct drawing at runtime
  };
}
```
...
public:
  ...
  void setBuffered(bool enable);                // enable/disable tiled double-buffered rendering
...
Actual-device rendering uses either direct drawing or tiled double-buffered drawing through LGFXVirtualCanvas according to the Export setting. Tiled double buffering is enabled by default and can be disabled at runtime with `screen.setBuffered(false)`. When generated for direct drawing, `setBuffered()` may be a no-op or may be omitted.
