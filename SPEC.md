...
public:
  explicit Screen(LovyanGFX& gfx) : Renderer(gfx, project) {}   // bind the descriptor
...
Because `Screen` binds the descriptor in its constructor, the user does not need to pass `project` every time. `setProfile()` names the `Profile` of the same namespace, so it is type-safe (passing another project's `Profile` is a compile error). The template syntax is exposed neither to the user nor to the library's public API.
...
GitHub Pages publishes `docs/` or the static artifacts generated by GitHub Actions.
