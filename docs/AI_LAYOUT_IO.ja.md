...
**Text** — 1 行テキスト。**Text には幅/高さの box がありません。** 折り返し、クリップ、省略、box 内アラインメントはなく、画面端を超えた文字は切れるだけです。収まる長さにし、はみ出す場合は profile ごとに、`text` を短くする・小さいフォント（例: `Font0`）や小さい `size` にする・`datum`/アンカーを調整する、のいずれかで画面内に収めます。
`x, y` は **アンカーポイント**です。`datum` はテキストのどの点をアンカーに置くかを表します。
`size` は **拡大率**です（描画高さ ≈ フォント基準高さ × `size`）。
`color` は `"#rrggbb"`、`text` は表示文字列です。固定ラベルも動的に変わる値も同じ `Text` で表します（専用の「Label」型はありません）。固定ラベルは確定文言、動的な値は実行時に差し替わるプレビュー値として `text` を入れます（§4 の静的レイアウトの境界）。
...
- **新規**の part には意味のある `id` を付ける（例: `text1` ではなく `battery`）。`id` は生成される `main.<id>` フィールド名になり、ユーザーの Arduino コードが呼ぶ。
- すべての profile を含む全体オブジェクトを、有効な JSON として返す。

**Don't**
- 既存の `id` を自分の判断で「改善」リネームしない。`main.<id>` フィールド名が変わりユーザーのコードが壊れる。意味の薄い id は、勝手に変えずユーザーにリネームを提案する。
