...
- **Fonts** モード（SPEC §8.7）: プリセットカタログ（`src/font-catalog.js`、`tests/font_introspect/gen.py`
  ...
  CJK はその言語自身でプレビュー（日本語 / 简体中文 / 繁體中文 / 한국어）。`tests/font_introspect`
...
- `src/font-catalog.js` — 生成済みプリセットカタログ（`../tests/font_introspect/gen.py` が生成）
