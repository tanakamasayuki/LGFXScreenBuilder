...
- **Fonts** mode (SPEC §8.7): browse the preset catalog (`src/font-catalog.js`,
  ...
  the pixel-exact `LGFXFontToolJs` LovyanGFX renderer (with an approximate family
  only while font data is loading). Codegen emits `setFont(&lgfx::v1::fonts::X)` per Text —
