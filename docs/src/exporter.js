...
import { ensureFontData, fontDataMap, customFontsOf, isCached } from './fontgen/build.js';
...
// fontData carries the generated bytes for custom fonts (§8.7.7). It is
// whatever the session cache holds right now; warmFonts() below rebuilds any
// stale entry and re-renders, so the preview settles on the real output.
const opts = () => ({
  profiles: [...included], buffered: buffered(), embedAiLayouts: embedAi(),
  fontData: fontDataMap(store.project),
});

// Custom fonts whose bytes are missing or out of date for the current recipe.
const staleFonts = () =>
  customFontsOf(store.project).filter(([name, recipe]) => !isCached(name, recipe));

// Rebuild stale custom fonts once, then re-render so the preview and the
// downloaded header both contain them. Runs at most one build at a time.
let warming = false;
async function warmFonts() {
  if (warming || !staleFonts().length) return;
  warming = true;
  $('export-fontstatus').textContent = t('cf.rebuilding');
  try {
    const { missing } = await ensureFontData(store.project, {
      onProgress: ({ name, done, total }) => {
        $('export-fontstatus').textContent = done
          ? t('cf.rebuildingOne', { name, done: done.toLocaleString(), total: total.toLocaleString() })
          : t('cf.rebuilding');
      },
    });
    $('export-fontstatus').textContent = missing.length
      ? t('cf.rebuildFailed', { list: missing.map((m) => m.name).join(', ') })
      : '';
  } finally {
    warming = false;
    renderExport();
...
export function renderExport() {
  ...
  // Generated fonts are rebuilt lazily: opening Export after loading a project
  // is the point where their bytes are needed again.
  warmFonts();
