...
// Characters above U+FFFF are dropped here rather than at selection time: the
// u8g2 format addresses glyphs with a uint16 encoding, so they can NEVER be
// generated. Leaving one in a curated set would mean every project that selects
// 常用漢字 is told about 𠮟 (U+20B9F) it can do nothing about. The BMP check
// survives for custom text and ranges, where the user can act on the warning.
const dropNonBmp = (cps) => cps.filter((c) => c <= 0xffff);

function encodeRanges(cps) {
  const sorted = [...new Set(dropNonBmp(cps))].sort((a, b) => a - b);
...
async function build() {
  ...
  for (const [k, v] of Object.entries(sets)) {
    ...
    const bmp = new Set(dropNonBmp([...uniq]));
    dropped += uniq.size - bmp.size;
    counts[k] = bmp.size;
  }
  if (dropped) console.error(`  (dropped ${dropped} codepoint(s) above U+FFFF — unrepresentable in u8g2)`);
  ...
  s += '// character.\n';
  s += '//\n';
  s += '// Characters above U+FFFF are excluded: the u8g2 format addresses glyphs with a\n';
  s += '// uint16 encoding, so they can never be generated and would only produce a\n';
  s += '// warning every project that selected them could do nothing about.\n';
